...
export function ProjectDetailModal({ project, tags, onClose, onEdit }: Props) {
  const relations = useDataStore(s => s.relations.filter(r => r.projectId === project.id))
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          {project.link && (
            <a href={project.link} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-sm text-primary hover:underline">
          ...
          {projectTags.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {projectTags.map(tag => <TagBadge key={tag.id} tag={tag} />)}
            </div>
          )}
            ...
            {linkedServices.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {linkedServices.map(service => (
                  <div key={service.id} className="flex items-center gap-2.5 rounded-md border border-border bg-muted/30 px-3 py-2">
                    ...
                    {service.link && (
                      <a href={service.link} target="_blank" rel="noopener noreferrer" className="ml-auto text-muted-foreground hover:text-foreground">
          ...
          {project.notes && (
            <>
              <Separator />
              <div>
                <h4 className="text-sm font-medium mb-2 flex items-center gap-1.5"><FileText className="h-3.5 w-3.5" /> Notas</h4>
                <p className="text-sm text-muted-foreground whitespace-pre-wrap rounded-md bg-muted/30 p-3">{project.notes}</p>
              </div>
            </>
          )}
          ...
          <div className="flex justify-end">
            <Button size="sm" variant="outline" onClick={() => onEdit(project)}><Edit className="h-3.5 w-3.5" /> Editar</Button>
          </div>
        </div>
      </DialogContent>
    </Dialog>
