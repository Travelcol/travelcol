...
export function TagsPage() {
  ...
  async function handleSubmit(data: FormData) {
    if (editingTag?.id) {
      await db.tags.update(editingTag.id, data)
      await logActivity('tag', 'updated', editingTag.id, data.name)
      toast.success('Tag actualizado')
    } else {
      const id = await db.tags.add({ ...data, createdAt: new Date().toISOString() })
      await logActivity('tag', 'created', id as number, data.name)
  ...
  async function handleDelete(id: number) {
    ...
    // Remove tag from projects and services
    const projects = await db.projects.toArray()
    for (const p of projects) {
      if (p.tags.includes(id)) {
        await db.projects.update(p.id!, { tags: p.tags.filter((tid) => tid !== id) })
      }
    }
    const services = await db.services.toArray()
    for (const s of services) {
      if (s.tags.includes(id)) {
        await db.services.update(s.id!, { tags: s.tags.filter((tid) => tid !== id) })
      }
    }
      ...
      <div className="flex-1 overflow-auto p-6">
        {tags.length === 0 ? (
          <EmptyState
            icon={Tags}
            title="No hay tags"
            description="Crea tags para organizar tus proyectos y servicios."
            action={
              <Button size="sm" onClick={openCreate}>
            ...
            <AnimatePresence>
              {tags.map((tag) => (
                <motion.div
                  ...
                  className="group flex items-center justify-between rounded-lg border border-border bg-card p-4 hover:border-border/80 transition-colors"
                >
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            Se eliminará el tag "{tag.name}" de todos los proyectos y servicios.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
