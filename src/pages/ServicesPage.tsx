...
export function ServicesPage() {
  ...
  function openEdit(service: Service) {
    ...
    setSelectedTagIds(service.tags)
    setDialogOpen(true)
  }
  ...
  async function handleSubmit(data: Omit<Service, 'id'>) {
    if (editingService?.id) {
      ...
      const id = await db.services.add(data)
      await logActivity('service', 'created', id as number, data.name)
  ...
  async function handleDelete(id: number) {
    ...
    await db.relations.where('serviceId').equals(id).delete()
    await db.nodePositions.where({ entityType: 'service', entityId: id }).delete()
  ...
  async function handleToggleFavorite(id: number) {
    const service = services.find((s) => s.id === id)
    if (!service) return
    await db.services.update(id, { isFavorite: !service.isFavorite })
  }
      ...
      <div className="flex-1 overflow-auto p-6">
        ...
        <div className="flex flex-wrap gap-3 mb-6">
          ...
          <Select value={providerFilter} onValueChange={setProviderFilter}>
            ...
            <SelectContent>
              ...
              {PROVIDERS.filter((p) => !usedProviders.includes(p)).map((p) => (
                <SelectItem key={p} value={p}>
                  {p}
                </SelectItem>
              ))}
        ...
        {filtered.length === 0 ? (
          <EmptyState
            ...
            description={
              services.length === 0
                ? 'Agrega tu primer servicio externo.'
